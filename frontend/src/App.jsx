...
import NormalizeDataView from "./components/NormalizeDataView.jsx";
...
const GROUP_RULES = [
  ...
  { group: "ECU_RPM", includes: ["ECU_RPM"] },
...
const App = () => {
  ...
  const [dataViewMode, setDataViewMode] = useState("rawData");
  ...
  return (
    <main className="layout">
      ...
      {activeTab === "analytics" && (
        <div>
          {/* Menu untuk Raw Data / Normalize Data */}
          <div style={{
            display: 'flex',
            gap: '8px',
            marginBottom: '12px',
            ...
            <button
              onClick={() => setDataViewMode("rawData")}
              style={{
                padding: '8px 16px',
                backgroundColor: dataViewMode === "rawData" ? "#3b82f6" : "transparent",
                color: dataViewMode === "rawData" ? "#fff" : "#cbd5e1",
                border: '1px solid #475569',
                borderRadius: '4px',
                cursor: 'pointer',
                fontWeight: dataViewMode === "rawData" ? 'bold' : 'normal'
              }}
            >
              Raw Data
            </button>
            <button
              onClick={() => setDataViewMode("normalizeData")}
              style={{
                padding: '8px 16px',
                backgroundColor: dataViewMode === "normalizeData" ? "#3b82f6" : "transparent",
                color: dataViewMode === "normalizeData" ? "#fff" : "#cbd5e1",
                border: '1px solid #475569',
                borderRadius: '4px',
                cursor: 'pointer',
                fontWeight: dataViewMode === "normalizeData" ? 'bold' : 'normal'
              }}
            >
              Normalize Data
            </button>
          ...
          {/* Render View sesuai dataViewMode */}
          {dataViewMode === "rawData" ? (
            <RawDataView
              telemetryRef={telemetryRef}
              ...
              logsList={logsList}
              listLogs={listLogs}
              loadOfflineReplay={loadOfflineReplay}
              importLocalCsv={importLocalCsv}
              playOfflineReplay={playOfflineReplay}
              pauseOfflineReplay={pauseOfflineReplay}
              seekOfflineReplay={seekOfflineReplay}
              clearOfflineReplay={clearOfflineReplay}
              offlineReplayStatus={offlineReplayStatus}
              offlineWindowSeconds={offlineWindowSeconds}
              setOfflineWindowSeconds={setOfflineWindowSeconds}
